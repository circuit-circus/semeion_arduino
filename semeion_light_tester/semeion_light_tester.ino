/*
  ...
  *** THIS VERSION IS FOR TESTING LIGHT EFFECTS WITH POTENTIOMETERS AND BUTTONS ***
*/
...
void readSensor(){
  proximity = map(analogRead(potOnePin), 0, 1024, 0, 20);  
  hue = map(analogRead(potTwoPin), 0, 1024, 0, 255);  
  }
...
void showLight() {
  ...
  for (int i = 0; i < NUM_LEDS; i++) {
    leds[i] = CHSV( hue, 187, brightness * 225);
  }
